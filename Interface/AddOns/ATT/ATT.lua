  ...
  --  C_SpecializationInfo.GetSpellsDisplay(63)
    ...
    -- Racials
        ...
        for abilityIndex, abilityTable in pairs(dbRacial) do
            ...
            if raceID ~= race or not db[ability] then ability = nil end
...
function ATT:UpdateAnchors(unit)
  --  local _, instanceType = IsInInstance()
  --  if db.showSelf and unit == "player" and instanceType == "none" then return end
...
function ATT:PLAYER_EQUIPMENT_CHANGED(item)
    ...
    if item ~= (13 or 14) then return end
    self:InspectPlayer()
end
...
function ATT:CreateOptions()
    ...
    local showCovenant = panel:MakeToggle(
        'name', 'Show Covenant',
        'description', 'Show Covenant abilities icons\n(beta state , doesnt work yet)',
        'default', false,
        'getFunc', function() return db.showCovenant end,
        'setFunc', function(value) db.showCovenant = value; self:UpdateGroup(); self:ApplyAnchorSettings() end)
    showCovenant:SetPoint("TOP",dampening,"BOTTOM",0,-5)
    ...
    showSelf:SetPoint("TOP",showCovenant,"BOTTOM",0,-5)
    ...
    authordesc:SetText("|cffffff00Version:|r |cff33ff99v"..ATTversion.."|r by |cffffff00izy|r  ( Contact: @curseforge.com - twitch.tv/imedia )")
...
function ATT:CreateAbilityEditor()
    ...
    local showPVETrinket = panel:MakeButton(
        ...
        'func', function()
            ...
            for abilityIndex, abilityTable in pairs(dbExtraTrinkets) do
                ...
                button.Text:SetText(" |T"..abilitytexture..":18|t " ..GetItemInfo(sname))
                button:SetHitRectInsets(0, -button.Text:GetWidth() , 0, 0)
                button:SetPoint("TOPLEFT",btn[#btn],"BOTTOMLEFT")
                button:SetChecked(db["Trinkets"][sname])
                button:SetScript("OnClick",  function() db["Trinkets"][sname] = button:GetChecked(); self:UpdateGroup(); self:ApplyAnchorSettings()  end)
                button:SetScript('OnEnter', function() GameTooltip:ClearLines(); GameTooltip:SetOwner(iFrame, "ANCHOR_TOP")
                    GameTooltip:SetSpellByID(id)
                end)
                button:SetScript('OnLeave', function()
    ...
    local selectRacials = panel:MakeButton(
        'name', 'Racials Tab',	
        'description', 'Open Racials Options',
        'extra', true,
        'func', function()
            ...
            for abilityIndex, abilityTable in pairs(dbRacial) do
                ...
                button:SetScript("OnClick",  function() db[ability] = button:GetChecked(); self:UpdateGroup(); self:ApplyAnchorSettings() end)
                button:SetScript('OnEnter', function() GameTooltip:ClearLines(); GameTooltip:SetOwner(iFrame, "ANCHOR_TOP")
                    GameTooltip:SetSpellByID(id)
                end)
                button:SetScript('OnLeave', function()
    ...
    local selectVisibility = panel:MakeButton(
        ...
        'func', function()
            ...
            for abilityIndex, abilityTable in pairs(dbVisibility) do
                ...
                button:SetWidth(30);
                if abilityIndex == 1 then btn[#btn+1] = 1 button:SetPoint("LEFT")  end
                button.text:SetText(" |T"..GetItemIcon(166879)..":18|t "..sname)
